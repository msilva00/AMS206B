...
\NormalTok{  p_new <-}\StringTok{ }\KeywordTok{lgamma}\NormalTok{(}\KeywordTok{sum}\NormalTok{(y[}\DecValTok{1}\OperatorTok{:}\NormalTok{m_new]) }\OperatorTok{+}\StringTok{ }\NormalTok{alpha) }\OperatorTok{-}\StringTok{ }\NormalTok{(}\KeywordTok{sum}\NormalTok{(y[}\DecValTok{1}\OperatorTok{:}\NormalTok{m_new]) }\OperatorTok{+}\StringTok{ }\NormalTok{alpha)}\OperatorTok{*}\KeywordTok{log}\NormalTok{(m_new }\OperatorTok{+}\StringTok{ }\NormalTok{beta) }\OperatorTok{+}\KeywordTok{lgamma}\NormalTok{(}\KeywordTok{sum}\NormalTok{(y[}\OperatorTok{-}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{m_new)]) }\OperatorTok{+}\StringTok{ }\NormalTok{gam) }\OperatorTok{-}\StringTok{ }\NormalTok{(}\KeywordTok{sum}\NormalTok{(y[}\OperatorTok{-}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\NormalTok{m_new)]) }\OperatorTok{+}\StringTok{ }\NormalTok{gam)}\OperatorTok{*}\KeywordTok{log}\NormalTok{(n}\OperatorTok{-}\NormalTok{m_new }\OperatorTok{+}\StringTok{ }\NormalTok{delta)}
  \CommentTok{# calculate acceptance probability and accept/reject acordingly}
\NormalTok{  accpt.prob <-}\StringTok{ }\KeywordTok{exp}\NormalTok{(p_new }\OperatorTok{-}\StringTok{ }\NormalTok{p_curr) }
  \ControlFlowTok{if}\NormalTok{(}\KeywordTok{runif}\NormalTok{(}\DecValTok{1}\NormalTok{) }\OperatorTok{<}\StringTok{ }\NormalTok{accpt.prob)}
\NormalTok{  \{}
\NormalTok{    m_curr <-}\StringTok{ }\NormalTok{m_new}
\NormalTok{  \}}
  \CommentTok{# save the current draws}
\NormalTok{  sample_save}\OperatorTok{$}\NormalTok{theta[i] <-}\StringTok{ }\NormalTok{theta_curr}
\NormalTok{  sample_save}\OperatorTok{$}\NormalTok{phi[i] <-}\StringTok{ }\NormalTok{phi_curr}
\NormalTok{  sample_save}\OperatorTok{$}\NormalTok{m[i] <-}\StringTok{ }\NormalTok{m_curr}
\NormalTok{\}}
\end{Highlighting}
\end{Shaded}

Using \(N=50000\) samples and a burnin of 5000, the results are plotted
below with the red line signifying the mean for the parameters

\includegraphics{AMS206b_H4_files/figure-latex/unnamed-chunk-34-1.pdf}
